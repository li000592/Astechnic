import "./IntroTwo.css";
export default function IntroTwo() {
  return (
    <section className="intro-two">
      <div className="two-right-container intro-two-container-1">
        <div className="two-title">
          INDUSTRIAL COMBUSTION SOLUTIONS<br></br>FOR YOUR INDUSTRY.
        </div>
        <div className="two-body">
          Astechinic digital solutions deliver advanced, actionable insights to help operators identify root causes,
          analyze performance and optimize their equipment and operations – for improved throughput, efficiency,
          reliability and safety.
        </div>
      </div>
      <div className="blue-brick-container">
        <div className="blue-brick-triangle" />
        <div className="two-right-container intro-two-container-2">
          <div className="two-title">
            DIGITAL SOLUTIONS FOR<br></br> YOUR INDUSTRY.
          </div>
          <div className="two-body">
            Astechnic industrial combustion deliver advanced, actionable insights to help operators identify root
            causes, analyze performance and optimize their equipment and operations–for improved throughput, efficiency,
            reliability and safety.
          </div>
        </div>
      </div>
    </section>
  );
}
